...
to complete computations including a derivation of the BSSN equations from the ADM
...
so that they can use Cadabra to do meaningful computations in general relativity. It was
written for readers with no prior knowledge of Cadabra and is presented as a series of
examples using familiar computations (such as verifying that the Levi-Civita connection is a
metric connection) as vehicles to present the various elements of Cadabra.
...
\Note{Maybe rewrite this paragraph to make clear that Cadabra can do item 2 and 3 above?}
...
\begin{cadabra}
   ...
   \partial{#}::PartialDerivative.                                           |\lstlabel{ex-01.04}|

   # Define a rule for the Christoffel symbol                                |\lstlabel{ex-01.c2}|

   Gamma := \Gamma^{a}_{b c} -> (1/2) g^{a d} (  \partial_{b}{g_{d c}}       |\lstlabel{ex-01.05}|
                                               + \partial_{c}{g_{b d}}       |\lstlabel{ex-01.06}|
                                               - \partial_{d}{g_{b c}} );    |\lstlabel{ex-01.07}|

   # Define the covariant derivative of the metric                           |\lstlabel{ex-01.c3}|

   cderiv := \partial_{c}{g_{a b}} - g_{a d}\Gamma^{d}_{b c}                 |\lstlabel{ex-01.08}|
                                   - g_{d b}\Gamma^{d}_{a c};                |\lstlabel{ex-01.09}|

   # Do the computations                                                     |\lstlabel{ex-01.c4}|

   substitute          (cderiv, Gamma);                                      |\lstlabel{ex-01.10}|
   distribute          (cderiv);                                             |\lstlabel{ex-01.11}|
   eliminate_metric    (cderiv);                                             |\lstlabel{ex-01.12}|
   eliminate_kronecker (cderiv);                                             |\lstlabel{ex-01.13}|
   canonicalise        (cderiv);                                             |\lstlabel{ex-01.14}|
\end{cadabra}
...
example (lines \lstref*{ex-01.c1}, \lstref*{ex-01.c2}, \lstref*{ex-01.c3},
\lstref*{ex-01.c4}). The statements in lines \lstref*{ex-01.01} to \lstref*{ex-01.04} assign
\emph{properties} to some symbols, while those in lines \lstref*{ex-01.05} to
\lstref*{ex-01.09} define two \emph{expressions} named \verb|Gamma| and \verb|cderiv|. The
...
will be manipulated in stages towards the final result (in line \lstref*{ex-01.14}). These
changes are obtained by applying Cadabra's algorithms (lines \lstref*{ex-01.10} to
\lstref*{ex-01.14}) to \verb|cderiv|. The other expression, \verb|Gamma|, defines a
...
algorithm in line \lstref*{ex-01.10}. This ability to defer the application of substitution
...
This feature also exists in MathTensor and xAct. % (but not so in GRTensorIII).
...
the final step of the calculation. The result in line \lstref{ex-01.14} is zero as expected.
...
It must be noted that identifiers carry no residual information about their origins. Once
...
But for rules there is a better solution -- they can be given names and they can
be concatenated. Here is a simple example of a named substitution rule
\begin{cadabra}
   RFromGamma  := R -> g^{a b} g^{c d} (    \partial_{c}{\Gamma_{a b d}}
                                          ...
                                          - \Gamma_{e a c} \Gamma^{e}_{b d} ).
\end{cadabra}
And here is that rule in action
\begin{cadabra}
   RicciScalar := R.
...
Python list). To achieve the same result as above you could use
\begin{cadabra}
   RicciScalar := R.
   substitute (RicciScalar, RFromGamma)
   substitute (RicciScalar, RFromGamma)
...
There is one caveat that must be noted -- the current version of Cadabra\footnote{Cadabra
2.2.7 (build 2166.d9aace25f4 dated 2019-09-21)} does not allow the addition of rules that
each contains just a single rule (as single rules are not \emph{lists}). A simple hack is to
convert each single rule to a list by adding a trivial rule, (e.g., replace \verb|foo->bah|
with \verb|{foo->bah,x->x}|).

Finally, you can also split named rules across lines, for example
\begin{cadabra}
   RFromRab := R ->
               R_{a b} g^{a b}.
   RabFromGamma := { R_{a b} ->
   ...
   RFromGamma = RFromRab \
              + RabFromGamma
\end{cadabra}
Though it must be admitted that this is an overkill (in this case). Note also the slash used
in the pair of lines used to build \verb|RFromGamma|. This is standard Python syntax
(without the slash Python will report an indentation error).
...
\begin{Exercises}
   ...
   \begin{Exercise}
      ...
      be devloped later in Exercise \exref{ex:4.06}.
   \end{Exercise}
...
line \lstref*{ex-05.103}. This shows that first and third terms where indeed left untouched
by \verb|substitute|.
...
to have say 10 terms and that the tag function have been used to target the $\mu_7$ term. If
some changes are made to the code preceeding that expression then it is possible that that
term may no longer be matched to $\mu_7$. The re-ordering of the terms might now find the
target term matched with $\mu_4$. This would require corresponding changes to the calls to
\verb|zoom|. It is also possible that this same problem could arise, not through any change
of the users code, but by changes made by the Cadabra team to the internal workings of its
own functions. The bottom lines is that the user must take care when using these functions
-- careful scrutiny of the output should be standard practice!
...
\end{cadabra}
A workaround for problems like this is given later in Exercise \exref{ex:6.10}.
...
\begin{cadabra}
   ...
   evaluate   (Gamma, gab+iab, rhsonly=True)                 |\lstlabel{ex-06.203}|
   evaluate   (Rabcd, gab+iab, rhsonly=True)                 |\lstlabel{ex-06.204}|
   evaluate   (Rab,   gab+iab, rhsonly=True)                 |\lstlabel{ex-06.205}|
\end{cadabra}
...
\begin{cadabra}
   ...
   compt := g_{a b} varphi^{a} varphi^{b}.   |\lstlabel{ex-06.401}|

   evaluate (compt,gab+basis)                |\lstlabel{ex-06.402}|
\end{cadabra}
The output from the above code is
\begin{align*}
...
\begin{Exercises}
   ...
   \begin{Exercise}
      ...
      {\bf Hint:} You may need to refer back to Exercise \exref{ex:6.10} to hide the first
...
\begin{cadabra}
   import cadabra2                                  |\lstlabel{ex:8.01}|
   from cadabra2_defaults import *                  |\lstlabel{ex:8.02}|
   __cdbkernel__ = cadabra2.__cdbkernel__           |\lstlabel{ex:8.03}|

   tmp = Indices(Ex(r'{a,b,c,d,e,f,g,h}'), Ex(r'position=independent)') ) |\lstlabel{ex:8.04}|
   tmp = RiemannTensor(Ex(r'R_{a b c d}'), Ex(r'') )                      |\lstlabel{ex:8.05}|
\end{cadabra}
...
\verb|RiemannTensor|). Lines \lstref*{ex:8.01} and \lstref*{ex:8.03} are mandatory while line
\lstref*{ex:8.02} is optional (without it the call to \verb|Ex| would need to be written as
\verb|cadabra2.Ex| as per standard Python syntax). Lines \lstref*{ex:8.04} and
\lstref*{ex:8.05} have been cleaned up slightly by replacing triple-quotes with single quotes
...
\begin{enumerate}
   ...
   downstairs when using \verb|Indices(position=independent)|. Thus the following code snippet
   will cause grief for Cadabra.
   \begin{cadabra}
      foo := A_{a} + B^{a}.
   \end{cadabra}
\end{enumerate}
...
\begin{enumerate}
   ...
   \Note{Check the lastest version of Cadabra to see if the above still applies.}
